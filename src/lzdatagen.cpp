...
std::map<size_t, unsigned char *> g_buf_tbl;

void
lzdg_generate_reuse_buffers(void *ptr, size_t size, double ratio, double len_exp, double lit_exp)
{
	bool match_in_table = false;
	if(g_buf_tbl.find(size) != g_buf_tbl.end()){
		match_in_table = true;
	}

	if(match_in_table){
		LOG_PRINT(LOG_DEBUG, "Match found in table reusing buffer\n");
		memcpy(ptr, g_buf_tbl[size], size);
	} else {
		LOG_PRINT(LOG_DEBUG, "Match not found in table, generating\n");
		void *saved_per_size_buffer = malloc(size);
		generate_data_internal(saved_per_size_buffer, size, ratio, len_exp, lit_exp, NULL);
		g_buf_tbl[size] = (unsigned char *) saved_per_size_buffer;
		memcpy(ptr, saved_per_size_buffer, size);
	}
}

void
free_reusable_buffers(void)
{
	for(auto it = g_buf_tbl.begin(); it != g_buf_tbl.end(); ++it){
		free(it->second);
	}
	g_buf_tbl.clear();
}
