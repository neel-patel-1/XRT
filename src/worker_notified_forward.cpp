...
void *dispatcher(void *arg){
  ...
  uint64_t failed_enq = 0;
  uint64_t warmup = num_requests / 10;
  uint64_t start, end;
  while(popped < warmup){
    if (q->try_dequeue(h)){
      #if defined(LATENCY)
  ...
  #endif
  #ifdef BOTTLENECK
  PRINT("[Dispatcher] Failed enqueues: %ld\n", failed_enq);
  #endif
...
void *worker(void *arg){
  ...
  while(popped < warmup){
    if(q->try_dequeue(r)){
      popped++;
      while(!c_q->try_enqueue(h.w_args));
    }
  }
  ...
  PRINT("[Worker %d] Processing Rate (MRPS): %f\n", id, (double)(num_requests-warmup) / ((double)(end - start) / 2100));
  #else
