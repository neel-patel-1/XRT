...
for(int j=0; j<iter; j++){
  ...
  for(int i=0; i<total_requests; i++){
    ...
    char *pointer_chain = (char *)&srcs[i * buf_size];
    ...
    // write_to_buf((char *)dst, buf_size);
      ...
      decrypt_feature((void *)pointer_chain, (void *)dst, buf_size);
    ...
    start = rdtsc();
    #endif
    dotproduct((void *)cpy_dst, (void *)&score, buf_size, &sz);
    #ifdef EXETIME
    end = rdtsc();
    #endif
