...
def find_latest_file(base_name):
    logs_dir = Path("logs")
    files = sorted(logs_dir.glob(f"{base_name}.?*"), key=lambda p: int(p.suffix[1:]), reverse=True)
    return str(files[0]) if files else str(logs_dir / f"{base_name}")

worker_file = find_latest_file("worker_notified_forward_summary.txt")
dispatcher_file = find_latest_file("worker_vs_dispatcher_summary.txt")
